...
export default function BriefFormPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (esEdicion) {
      ...
      // Calcular número dentro del batch
    ...
    if (hooksConTexto.length > 0) {
      const { error } = await supabase.from("hooks").insert(hooksConTexto)
      ...
      <form onSubmit={handleSubmit} style={{ display: "grid", gap: "1.25rem" }}>
        ...
        <div className="action-bar">
          <button type="submit" className="btn btn-primary" disabled={guardando || !batchId}>
            {guardando ? "Guardando..." : esEdicion ? "Guardar cambios" : "Crear brief"}
          </button>
